...
sns.boxplot(x=mtcars_df["hp"])

# affichage de la valeur extrême
print("Max 'hp':", mtcars_df["hp"].max())
mtcars_df[mtcars_df["hp"]==mtcars_df["hp"].max()]

# supprimons la ligne 30 dans le jeu de données initial
mtcars_df = mtcars_df.drop(index=30)
mtcars_df.shape # ligne 30 retirée
...
scores['prediction'] = (y_predict_train.reshape(-1,1).round(2),
                y_predict_test.reshape(-1,1).round(2))
scores['r2'] = (r2_train.round(3),
                  r2_test.round(3))
scores['rmse'] = (rmse_train,
                  rmse_test)
scores_df = pd.DataFrame(scores).transpose()
scores_df.columns = ['Train', 'Test']
print(scores_df)
